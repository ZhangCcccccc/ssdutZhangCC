...
<script type="text/javascript">
	...
	$(function(){
		...
		scheduler.config.lightbox.sections = [	 
			{name:"phone", height:21, map_to:"phone", type:"textarea"},
		...
		scheduler.attachEvent("onEventAdded", function(event_id,event_object){
			...
			if(event_object.start_date.Format("yyyy-MM-dd")<new Date(year,month,day,hour,minute,second).Format("yyyy-MM-dd")){
			   ...
			   scheduler.deleteEvent(event_id);
				...
				$.post('${pageContext.request.contextPath}/device/saveDeviceReservation',{equinemtid:equinemtid,startDate:startdate,endDate:enddate,description:description,phone:phone,property:property,research:research},function(data){
			        if(data == 'success') {
			        	/*if ("${device.CDictionaryByTeacherAudit.CCategory=='c_active' && device.CDictionaryByTeacherAudit.CNumber=='1'&&device.isAuditTimeLimit!=null&&device.isAuditTimeLimit==1&&device.auditTimeLimit!=null &&device.auditTimeLimit!=''}") {
			        ...
			        if(data == 'LIMIT'){
			            $.messager.alert('错误','你预约的时间在实验室禁用期间!','error');
			            scheduler.deleteEvent(event_id);
