...
<div class="right-content">	
	...
	<!-- 标题栏  -->
	  <ul class="TabbedPanelsTabGroup">
			...
			<a href="${pageContext.request.contextPath}/device/labManagerReservationAudit?id=${reservation.id}">实验室管理员审核</a>
			</li>
	 ...
	 <!--内容栏  --> 
	  <div class="TabbedPanelsContentGroup">
		<div class="TabbedPanelsContent">
			...
			<%-- <c:if test="${reservation.labRoomDevice.CActiveByLabManagerAudit.id==1}"> --%>  <!-- 需要实验室管理员审核-->
			...
			<c:if test="${isUnderLabManagerAudit && reservation.CAuditResult.CCategory=='c_audit_result' && reservation.CAuditResult.CNumber=='1'}">
				<c:if test="${fn:contains(reservation.labRoomDevice.labRoom.labRoomAdmins,username)}">  <!-- 当前登陆人是实验室管理员  -->
					<div class="title">实验室管理员意见</div>
				...
				<c:if test="${empty labManageAudit && fn:indexOf(reservation.labRoomDevice.labRoom.labRoomAdmins,user.username)==-1}"> <!-- 未有实验室管理员审核意见，且当前登录人不是实验室管理员-->
					<table id="listTable" width="50%" cellpadding="0" cellspacing="0" class="tablesorter" >
					    <tr><td colspan="6">实验室管理员信息 </td></tr>
			...
			<c:if test="${reservation.stage>1 || (reservation.stage==1 && reservation.labRoomDevice.CDictionaryByTeacherAudit.CCategory=='c_active' &&reservation.labRoomDevice.CDictionaryByTeacherAudit.CNumber=='2')
			 ||reservation.stage==-1}"> <!-- 审核阶段高于当前阶段，或者审核拒绝 -->	
				<table id="listTable" width="50%" cellpadding="0" cellspacing="0" class="tablesorter">
						  <tr><td colspan="6" ><font color="blue">实验室管理员审核</font> </td></tr>
						  ...
						  <c:if test="${empty labManageAudit }">
						  	<tr>
							  <td>
							  	<font color="red">没有实验室管理员审核意见。</font>
							  </td>
						  	</tr>
						  </c:if>		
				</table>
		...
		<%-- <c:if test="${reservation.labRoomDevice.CActiveByLabManagerAudit.id!=1}"> --%>  <!-- 不需要实验室管理员审核-->
		<c:if test="${reservation.labRoomDevice.CDictionaryByLabManagerAudit.CCategory=='c_active' &&reservation.labRoomDevice.CDictionaryByLabManagerAudit.CNumber!='1'}">  
			<table id="listTable" width="50%" cellpadding="0" cellspacing="0" class="tablesorter">
						  <tr><td colspan="6"><font color="blue">实验室管理员审核情况</font> </td></tr>
								<tr>
									<td>您好，该设备不需要实验室管理员审核。</td>
								</tr>
			</table>
		</c:if>
