...
<html>
	...
	src="${pageContext.request.contextPath}/js/operation/newoperationoutlineMyAudit.js"></script>
	...
	<body>
		<div id="bgheight">
			...
			<div class="list_chose">
				<div class="list_chose_div all_btn"><a href="${pageContext.request.contextPath}/newoperation/experimentalAudit?currpage=1">全部审核</a></div>
				<div class="list_chose_div my_btn selected_btn"><a href="${pageContext.request.contextPath}/newoperation/experimentalMyAudit?currpage=1">我的审核</a></div>
			</div>
			<div class="all_main_content">
				<div class="all_div">
					<div class="gray_line">
							<form:form name="form"
								action="${pageContext.request.contextPath}/newoperation/experimentalMyAudit?currpage=1"
					...
					<div class="bgwo w100p ptb10">
						<table class="experimental_list_tab" cellspacing="0">
							...
							<c:forEach items="${Outlinelist}" varStatus="i" var="s">
								...
								<a class="fa fa-search blue" title="查看详情" href="${pageContext.request.contextPath}/newoperation/viewOperationOutline?idkey=${s.id}&from=2&page=${currpage}"></a>
								<c:if test="${s.status eq 1 }">
								<a class="fa fa-check-circle blue" title="通过" href="${pageContext.request.contextPath}/newoperation/myAuditPassOutline?outlineId=${s.id}"></a>
									<a class="fa fa-times-circle blue" title="拒绝" href="${pageContext.request.contextPath}/newoperation/myAuditRefuseOutline?outlineId=${s.id}"></a>
								</c:if>
								</td>
								<c:if test="${s.status eq 1 }">
						...
						<div class="page-select">
							...
							<form>
							   ...
							   <option value="${pageContext.request.contextPath}/operation/experimentalMyAudit?currpage=${currpage}">${currpage}</option>
							   <c:forEach begin="${pageModel.firstPage}" end="${pageModel.lastPage}" step="1" varStatus="j" var="current">	
					           <c:if test="${j.index!=currpage}">
					           <option value="${pageContext.request.contextPath}/operation/experimentalMyAudit?currpage=${j.index}">${j.index}</option>
					           </c:if>
					           </c:forEach>
					           </select>
							</form>
		...
		<script type="text/javascript">
			function cancel(){
  				window.location.href="${pageContext.request.contextPath}/newoperation/experimentalMyAudit?currpage=1";
			}
			$(".all_btn").click(
				function() {
					window.location.href="${pageContext.request.contextPath}/newoperation/experimentalAudit?currpage=1";
				}
			);
			$(".my_btn").click(
				function() {
