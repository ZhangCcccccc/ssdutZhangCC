	...
	<body>
		<div id="bgheight">
			...
			<div class="all_main_content">
				<div class="grey_line">
				<form:form name="form"
								action="${pageContext.request.contextPath}/newoperation/viewCourseOperationOutlines?currpage=1"
				...
				<table class="experimental_list_tab" cellspacing="0">
					...
					<c:forEach items="${Outlinelist}" varStatus="i" var="s">
						...
						<td class="tc">
							<a class="fa fa-search blue" title="查看详情" href="${pageContext.request.contextPath}/newoperation/viewOperationOutline?idkey=${s.id}"></a>
						</td>
						</tr>
					</c:forEach>
				</table>
				<div class="page-message">
						...
						<div class="page-select">
							...
							<form>
							   ...
							   <option value="${pageContext.request.contextPath}/newoperation/viewCourseOperationOutlines?currpage=${currpage}">${currpage}</option>
							   <c:forEach begin="${pageModel.firstPage}" end="${pageModel.lastPage}" step="1" varStatus="j" var="current">	
					           <c:if test="${j.index!=currpage}">
					           <option value="${pageContext.request.contextPath}/newoperation/viewCourseOperationOutlines?currpage=${j.index}">${j.index}</option>
					           </c:if>
					           </c:forEach>
					           </select>
							</form>
		...
		<script type="text/javascript">
			...
			function cancel(){
  				window.location.href="${pageContext.request.contextPath}/newoperation/viewCourseOperationOutlines?currpage=1";
			}
		//首页
function first(){
	document.form.action="viewCourseOperationOutlines?currpage=1";
...
function previous(){
    ...
    document.form.action="viewCourseOperationOutlines?currpage="+page;
    document.form.submit();
}
