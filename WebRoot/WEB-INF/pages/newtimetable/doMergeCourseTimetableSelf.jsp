	...
	<body>
				...
				<div class="w25p r message_container">
					<div class="public_message">
						<div class="pub_tit">
							...
							<button class="r" type="button" onclick="completeAll('${mergeId}')">全部完成</button>
							</c:if>
							<c:if test="${isPartComplete eq 1 }">
							<button class="r" type="button" onclick="modifyAll('${mergeId}')">修改</button>
							</c:if>
						</div>
							...
							<c:set var="has_timetable" value="0"></c:set>
								...
								<div class="course_select cf" id="course_select${group.id }">
									<div>
										...
										<span>人数：</span>
										<input type="text" class="check_text" id="total${group.id }" id="total${group.id }" value="${group.numbers}" readonly/>
									...
									<c:set var="has_timetable" value="1"></c:set>
										...
										<c:if test="${isThisItem == 1}">
											...
											<div class="course_basic_message cf"  id="sample${curr.id }">
												<div class="cbm_box">
													<div>实验室：</div>
												...
												<div class="cbm_box" id="teacher-chzn">
													<div>教师：</div>
													<select id="teacher${curr.id }"  multiple="true" class="chzn-select"  onchange="saveTeacher(${curr.id })"><!-- 此处增加 multiple="true" 会走样 -->
														<option value="" >课程组</option>
									...
									<c:if test="${has_timetable eq 0}">
	                                    <button class="r mt5 mb10 mr5p" type="button" onclick="delGroupMerge(${mergeId},${type}, ${item})">取消分批</button>
	                                </c:if>
		...
		// 学生判冲
		 function generateConflict(){
			...
			function modifyAll(){
				$.ajax({
			         url:"${pageContext.request.contextPath}/newtimetable/modifyMergeTimetable?mergeId=${mergeId}",
			         dataType:"json",
			         type:'GET',
			         complete:function(result)
			         {
