	...
	<body>
			...
			<div class="all_main_content ovh">
				<div class="w74p l">
					<div class="normal">
						<div class="gray_line">
							<form name="form"
								action="${pageContext.request.contextPath}/newtimetable/doSpecializedBasicCourseTimetable?courseDetailNo=${courseDetailNo}"
								method="post">
					...
					<div class="public_message">
						...
						<div class="track_detail">
							<div class="course_method cf" style="width:90%;height:auto;">
								<form>
									<div class="l">
										...
										<input type="radio" checked="checked" class="student_course" name="course_method" onclick="changeStyle(1)" />
										<span>学生自选</span>
									</div>
									<div class="l">
							...
							<div class="student_select">
								...
								<c:if test="${group.timetableStyle == 21 && group.labRoom.id == labRoom.id }">
								<div class="course_select cf" id="course_select${group.id }">
									<div>
										...
										<span>人数：</span>
										<input type="text" class="check_text" value="${group.numbers}"id="changegroup1" <%-- onblur="savegroup1(${group.id})" --%>/>
								...
								<c:if test="${group.timetableStyle != 21 || group.labRoom.id != labRoom.id }">
									<div class="course_select cf" id="course_select${group.id }">
										<div>
											...
											<span class="b o1">${group.groupName }&nbsp;&nbsp;&nbsp;</span>
											<span>人数：</span>
											<input type="text" class="check_text" value="${group.numbers}" id="changegroup2" <%-- onblur="savegroup2(${group.id})" --%>/>
											<!-- <i class="fa fa-times r close" title="关闭"></i> -->
										</div>
									</div>
								</c:if>
							...
							<div class="student_select">
								<div class="pub_select">
									<div>
										<span class="pr5">选课开始:</span>
										<fmt:formatDate value="${schoolCourseDetail.courseStart.time}" type="date" pattern="yyyy-MM-dd HH:mm:ss"/>
									</div>
									<div>
										<span class="pr5">选课结束:</span>
										<fmt:formatDate value="${schoolCourseDetail.courseEnd.time}" type="date" pattern="yyyy-MM-dd HH:mm:ss"/>
									</div>
									<!--<div>
								<span class="pr5">选课结束:</span>
								<span>每节课开始前</span>
								...
								<c:if test="${group.timetableStyle != 21 || group.labRoom.id != labRoom.id }">
									<div class="course_select cf" id="course_select${group.id }">
										<div>
											...
											<span class="b o1">${group.groupName }&nbsp;&nbsp;&nbsp;</span>
											<span>人数：</span>
											<input type="text" class="check_text" value="${group.numbers}"id="changegroup3" <%-- onblur="savegroup3(${group.id})" --%>/>
											<!-- <i class="fa fa-times r close" title="关闭"></i> -->
										</div>
									</div>
								</c:if>
							...
							<div class="student_select">
								...
								<div class="course_select cf" id="course_select${group.id }">
									<div>
										...
										<span class="b o1">${group.groupName }&nbsp;&nbsp;&nbsp;</span>
										<span>人数：</span>
										<input type="text" class="check_text" id="changegroup" name="changegroup" value="${group.numbers}"<%--  onblur="savegroup(${group.id})" --%>/>
										<!-- <i class="fa fa-times r close" title="关闭"></i> -->
									</div>
									<c:forEach items ="${ group.timetableAppointments}" var="curr">
										<c:set var="isThisItem" value="0"/>
		...
		$(document).ready(function(){
			...
			function saveStartTime(){
				$.ajax({
			         url:"${pageContext.request.contextPath}/newtimetable/saveStartTime?startTime="+$("#basicCourseStart").val()+"&courseDetailNo=${courseDetailNo}",
			         dataType:"json",
			         type:'GET',
			         complete:function(result)
			         {
			...
			function saveEndTime(){
				$.ajax({
			         url:"${pageContext.request.contextPath}/newtimetable/saveEndTime?endTime="+$("#basicCourseEnd").val()+"&courseDetailNo=${courseDetailNo}",
			         dataType:"json",
			         type:'GET',
			         complete:function(result)
			         {
		...
		function saveActualStartDate(groupId,appointmentId){
			...
			function completeAll(){
				$.ajax({
			         url:"${pageContext.request.contextPath}/newtimetable/completeDetailTimetable?courseDetailNo=${courseDetailNo}",
			         dataType:"json",
			         type:'GET',
			         complete:function(result)
			         {
			...
			function cancel(){
				window.location.href = "${pageContext.request.contextPath}/newtimetable/doSpecializedBasicCourseTimetable?courseDetailNo=${courseDetailNo}";
			}
			/* function saveGroupNums(num, groupId){
				//教师先不管
				$.ajax({
			         url:"${pageContext.request.contextPath}/newtimetable/saveGroupNums",
			         type:'POST',
			          data:{groupId:groupId,num:num},
		            error:function (request){
		            	 alert('请求错误!');
		            },
					success:function(result)
			...
			function sendMessageAll(){
				$.ajax({
			         url:"${pageContext.request.contextPath}/newtimetable/sendDetailMessageAll?courseDetailNo=${courseDetailNo}",
			         dataType:"json",
			         type:'GET',
			         complete:function(result)
			         {
			...
			function sendMessageByBacth(){
				$.ajax({
			         url:"${pageContext.request.contextPath}/newtimetable/sendDetailMessageByBatch?courseDetailNo=${courseDetailNo}&batchId="+${batchId},
			         dataType:"json",
			         type:'GET',
			         complete:function(result)
			         {
			...
			function savegroup(groupId){
				var domInput = document.getElementById("changegroup");
				var number =domInput.value;					
			
	            $.ajax({
		        async: false,
		        type: "POST",
		        url: "${pageContext.request.contextPath}/newtimetable/savegroup?groupId="+groupId+"&courseDetailNo=${courseDetailNo}&number="+number,
		        dataType:'html',
		        success:function(data){
		        alert("成功修改");
	            	},
			...
			function savegroup1(groupId){
				var domInput = document.getElementById("changegroup1");
				var number =domInput.value;	
	            $.ajax({
		        async: false,
		        type: "POST",
			...
			function savegroup2(groupId){
				var domInput = document.getElementById("changegroup2");
				var number =domInput.value;	
	            $.ajax({
		        async: false,
		        type: "POST",
			...
			function savegroup3(groupId){
				var domInput = document.getElementById("changegroup3");
				var number =domInput.value;
	            $.ajax({
		        async: false,
		        type: "POST",
			...
			//function turnPage(){
				//track.goToPage(3);
				//document.form.action = "${pageContext.request.contextPath}/newtimetable/listMySchedule?currpage=1";
			//}
		</script>
