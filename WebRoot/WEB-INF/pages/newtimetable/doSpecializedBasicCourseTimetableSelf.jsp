...
<html>
	...
	<body>
		<div id="bgheight">
			...
			<form name="form"
								action="${pageContext.request.contextPath}/newtimetable/doSpecializedBasicCourseTimetableSelf?courseDetailNo=${courseDetailNo}"
								method="post">
						...
						<div class="grey_line bgw bg6d">
							<table>
								<tr>
									...
									<td class="b wh ts ls1 hide" id="addHint">移动学生</td>
								</tr>
							</table>
						...
						<div class="track_detail">
							<div class="course_method cf" style="width:90%;height:auto;">
								<form>
									...
									<div class="l">
										<input type="radio"  checked="checked" class="teacher_course" name="course_method" onclick="changeStyle(3)"/>
										<span>自主分批</span>
									</div>
									<div class="l">
							...
							<!-- 有无排课 -->
							<c:set var="has_timetable" value="0"></c:set>
								...
								<div class="course_select cf" id="course_select${group.id }">
									<div>
										...
										<span>人数：</span>
										<input type="text" class="check_text" value="${group.numbers}" id="total${group.id }" name="total${group.id }" readonly/>
									</div>
									<c:forEach items ="${ group.timetableAppointments}" var="curr">
									<c:set var="has_timetable" value="1"></c:set>
										<c:set var="isThisItem" value="0"/>
											...
											<c:forEach items="${curr.timetableItemRelateds }" var="selectItem">
												<c:if test="${empty selectItem.operationItem.id }">
												<c:set var="isThisItem" value="1"/>
											</c:if>
											</c:forEach>
							...
							<div class="teacher_select hide">
									<c:forEach items="${groups }" var="group" varStatus="i">
										<div class="course_select cf" id="course_select${i.count }">
											<div>
												<input type="checkbox" class="check_btn" name="checkGroup" value="${i.count }" onclick="check(this)"/>
												<span class="b o1">第一批第${i.count }组&nbsp;&nbsp;&nbsp;</span>
												<span>人数：</span>
											<input type="text" class="check_text" value="${group.numbers}" id="total${group.id }" name="total${group.id }" readonly/>
											...
											<div class="course_basic_message">
												...
												<div class="cbm_box">
													<div>星期：</div>
													<input class="input_text" value="111"/>
												</div>
												<div class="cbm_box">
													<div>周次：</div>
													<input class="input_text" value="111"/>
											...
											<span class="student_list" title="点击查看学生名单">查看学生名单</span>
										</div>
										</c:forEach>
							</div>
							...
							<div class="w100p ovh">
								...
								<c:if test="${isPartComplete eq 1 }">
									<button class="r mt5 mb10 mr5p" type="button" onclick="sendDetailMessageByGroup('${courseDetailNo}')">发送消息</button>
		...
		// 学生判冲
		 function generateConflict(){
			 ...
			 var groupId = $("#groupId").val();
              $.ajax({
                    ...
                    success:function(data){//AJAX查询成功
                        if($("#total"+groupId).val()!=null && $("#total"+groupId).val()!=''){
                            $('.choose_this').each(function(){
                                //$(this).html("0/"+$("#total"+groupId).val());
                        ...
                        for(var t in data) {
                             ...
                             if($("#"+time).hasClass("choose_this"))
                             {
                                 ...
                                 var totalNumber = 0;
                                 if($("#total"+groupId).val()!=null && $("#total"+groupId).val()!=''){// 当前组下的人数
                                     totalNumber = $("#total"+groupId).val();
                                 }else{// 实验室容量
			...
			function changeStudents(){
					form.action= "${pageContext.request.contextPath}/newtimetable/changeStudents?type=1";
					form.submit();
			}
                        ...
                        if(data=="yes"){// 可以复制
                            $.messager.confirm('提示','是否要复制已存在的分批？',function(r){
                                if(r) {// 若选择复制，则弹框
                                    ...
                                    document.form.action="${pageContext.request.contextPath}/newtimetable/doSpecializedBasicCourseTimetableSelf?courseDetailNo=${courseDetailNo}"
			...
			function modifyAll(){
				$.ajax({
			         url:"${pageContext.request.contextPath}/newtimetable/modifyDetailTimetable?courseDetailNo=${courseDetailNo}",
			         dataType:"json",
			         type:'GET',
			         complete:function(result)
			         {
			...
			function cancel(){
				window.location.href = "${pageContext.request.contextPath}/newtimetable/doSpecializedBasicCourseTimetableSelf?courseDetailNo=${courseDetailNo}";
			}
			
			function sendMessageAll(){
				$.ajax({
			         url:"${pageContext.request.contextPath}/newtimetable/sendMessageAllNotSelfSelect?courseDetailNo=${courseDetailNo}",
			         dataType:"json",
			         type:'GET',
			         complete:function(result)
			         {
