	...
	<body>
		<div id="bgheight">
			...
			<div class="all_main_content">
				<div class="gray_line">
				<form:form name="form"
								action="${pageContext.request.contextPath}/newtimetable/listLabSchoolCourseDetailPermissions?currpage=1"
					...
					<table class="experimental_list_tab" cellspacing="0">
						...
						<c:forEach items="${courseDetailList }" var="curr" varStatus="i">
							<tr>
								...
								<td class="tc">
								<c:forEach items="${curr.userByScheduleTeachers}" var="curr1">
								<span style="cursor:pointer" onclick="deletescheduleTeacher(${i.count},'${curr1.username}')">${curr1.cname}</span>
								</c:forEach>
								<input id="getscheduleTeacher${i.count}" class="icount" name="getscheduleTeacher${i.count}" />
									<input type="button"  value="确认" onclick="saveScheduleTeacher(${i.count});">
									</td>
								</tr>
						</c:forEach>
					</table>
					<div class="page-message">
						...
						<div class="page-select">
							...
							<form>
							   ...
							   <option value="${pageContext.request.contextPath}/newtimetable/listLabSchoolCourseDetailPermissions?currpage=${currpage}">${currpage}</option>
							   <c:forEach begin="${pageModel.firstPage}" end="${pageModel.lastPage}" step="1" varStatus="j" var="current">	
					           <c:if test="${j.index!=currpage}">
					           <option value="${pageContext.request.contextPath}/newtimetable/listLabSchoolCourseDetailPermissions?currpage=${j.index}">${j.index}</option>
					           </c:if>
					           </c:forEach>
					           </select>
							</form>
...
//首页
function first(){
	document.form.action="listLabSchoolCourseDetailPermissions?currpage=1";
...
function previous(){
    ...
    document.form.action="listLabSchoolCourseDetailPermissions?currpage="+page;
    document.form.submit();
}
...
function last(){
	...
	function cancel(){
  				window.location.href="${pageContext.request.contextPath}/newtimetable/listLabSchoolCourseDetailPermissions?currpage=1";
			}
	...
	function changeScheduleTeacher(number){
		...
		$.ajax({
			         url:"${pageContext.request.contextPath}/newtimetable/changeScheduleTeachers",
			         dataType:"json",
			         type:'GET',
			         data:{detailNo:detailNo,teacher:teacher.join(",")},
	...
	function saveScheduleTeacher(number)
	{
		var teacher = $("#getscheduleTeacher"+number).val().replace(/[^0-9]/ig,"");
		if(teacher != null){
			var detailNo = $("#detailNo"+number).val();
			window.location.href="${pageContext.request.contextPath}/newtimetable/changeScheduleTeachers?detailNo="+detailNo+"&teacher="+teacher;
		}
		
	}
	function deletescheduleTeacher(number,name){
		var detailNo = $("#detailNo"+number).val();
		window.location.href="${pageContext.request.contextPath}/newtimetable/deletescheduleTeacher?name="+name+"&detailNo="+detailNo;
	}
		</script>
		...
		<script
				...
				<script type="text/javascript" src="${pageContext.request.contextPath}/js/jquery.coolautosuggest.js"></script>
					...
					//输入框绑定的方法
					$(".icount").coolautosuggest({
						url:"${pageContext.request.contextPath}/newtimetable/coolSuggestDevice?userName=",
