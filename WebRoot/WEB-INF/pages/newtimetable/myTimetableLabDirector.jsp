	...
	<body>
		<div id="bgheight">
			...
			<div class="all_main_content">
				<div class="gray_line">
				<form name="form"
								action="${pageContext.request.contextPath}/newtimetable/myTimetableAdmin"
								method="post">
					<table style="width:400px;">
						<tr>
							<td>
								<span>学期:&nbsp;</span>
								<select name="term" class="chzn-select" style="width:300px;">
					...
					<table style="width:400px;">
						<tr>
							<td>
								<span>课程:&nbsp;</span>
								<select name="courseId" class="chzn-select" style="width:300px;">
									<option value="">请选择</option>
									<c:forEach items="${courses}" var="curr">
									<c:if test="${courseId != curr[1]}">
										<option value="${curr[1]}">${curr[2]}${curr[1]}</option>
									</c:if>	
									<c:if test="${curr[1] == courseId}">
										<option value="${curr[1]}" selected="selected">${curr[2]}${curr[1]}</option>
									</c:if>
									</c:forEach>
								</select>
							</td>
							       		...
							       		<tr>
								            ...
								            <c:forEach items="${schoolTermWeeks }" var="currWeek1" varStatus="i">
								            	...
								            	<c:forEach items="${appointments}" var="currTimetable">
								            		...
								            		<c:if test="${currTimetable.timetableItemRelateds ne null}">
									            	...
									            	<c:if test="${currItem.schoolCourseDetail ne null}">
									            		<c:set var="codeCustom" value="${currItem.schoolCourseDetail.courseNumber}-"/>
									            	</c:if>
									            	<c:if test="${currItem.schoolCourseMerge ne null}">
									            		<c:set var="codeCustom" value="${currItem.schoolCourseMerge.courseNumber}-"/>
									            	</c:if>
								            		...
								            		<c:if test="${currTimetable.timetableAppointmentSameNumbers eq null || fn:length(currTimetable.timetableAppointmentSameNumbers) == 0 }">
									            		...
									            		&& cStatus2.index > currTimetable.startClass/2 && cStatus2.index <= currTimetable.endClass/2}">
								            					...
								            					<c:if test="${currTimetable.timetableTeacherRelateds ne null && currTimetable.schoolCourseDetail ne null}">
									            				...
									            				<c:if test="${type == 1 }">
									            					<c:if test="${not empty currTimetable.schoolCourseMerge}">
										            					<c:set var="courseNumber" value="${currTimetable.schoolCourseMerge.courseNumber }<br>${teachers}"/>
										            				</c:if>
										            				<c:if test="${currTimetable.timetableStyle != 24 && currTimetable.timetableStyle != 25 && currTimetable.timetableStyle != 26}">
										            					<c:set var="courseNumber" value="${currTimetable.schoolCourseDetail.courseNumber }<br>${teachers}"/>
										            				</c:if>
									            				</c:if>
									            				<c:if test="${type == 2 }">
		...
		<script type="text/javascript">
			...
			function cancel(){
				window.location.href = "${pageContext.request.contextPath}/newtimetable/myTimetableLabDirector";
			}
			...
			function query(){
				document.form.action="${pageContext.request.contextPath}/newtimetable/myTimetableLabDirector";
				document.form.submit();
			}
