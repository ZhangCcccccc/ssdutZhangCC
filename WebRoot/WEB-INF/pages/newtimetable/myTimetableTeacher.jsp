	...
	<head>
		...
		<script type="text/javascript">
			function getCourseByTerm(){
				...
				$.ajax({
					type: "POST",
					url: "${pageContext.request.contextPath}/newtimetable/getCourseByTermTeacher",
					data: {termId:term},
		...
		<script type="text/javascript">
			function showAllSchedule(){
				...
				if(courseId==""){
					...
					document.form.action='${pageContext.request.contextPath}/newtimetable/myTimetableTeacherAll',
	...
	<body>
		<div id="bgheight">
			...
			<div class="all_main_content">
				<div class="gray_line">
				<form name="form" action="${pageContext.request.contextPath}/newtimetable/myTimetableTeacher" method="post">
					...
					<table style="width:400px;">
						<tr>
							<td>
								<span>课程:&nbsp;</span>
								<select id="courseId" name="courseId" class="chzn-select" style="width:300px;">
									<option value="">请选择</option>
									<c:forEach items="${courses}" var="curr">
									<c:if test="${courseId == curr.courseDetailNo}">
										<option value="${curr.courseDetailNo}" selected="selected">${curr.schoolCourse.courseName}${curr.schoolCourse.courseNo}</option>
									</c:if>
									<c:if test="${courseId != curr.courseDetailNo}">
										<option value="${curr.courseDetailNo}">${curr.schoolCourse.courseName}${curr.schoolCourse.courseNo}</option>
									</c:if>	
									</c:forEach>
								</select>
							</td>
						...
						<tr>
							<td>
								...
								<select name="academy" class="chzn-select" style="width:300px;">
									<option value="">请选择</option>
									<c:forEach items="${mapAcademy}" var="curr">
									<c:if test="${academy == curr.key}">
					...
					<table>
						<tr>
							<td>
								...
								<button>取消</button>
								<a onclick="showAllSchedule();return false;"><button>查看全部</button></a>
							</td>
						</tr>
					</table>
					</form>
					...
					<table class="timetable_tab" cellspacing="0" id="myShow">
						...
						<c:forEach var="class1"  varStatus="cStatus1" begin="${schoolTermWeeks[0][10]}" end="7">
							       	<c:forEach var="class2" varStatus="cStatus2" begin="1" end="5">
							       		<tr>
							       			<c:if test="${cStatus2.index==1}"><!-- 第一列 -->
								            ...
								            <c:forEach items="${schoolTermWeeks }" var="currWeek1" varStatus="i">
								            	...
								            	<c:set var="startDate" value="0"/>
								            	<c:set var="endDate" value="0"/>
								            	<c:set var="sameCount" value="0"/>
								            	<c:forEach items="${appointments}" var="currTimetable">
								            		<c:set var="thisGroup" value="0"/>
								            		<c:set var="classNumber" value="0"/>
								            		<c:forEach items="${currTimetable.timetableGroups }" var="currGroup">
								            			<c:set var="thisGroup" value="${currGroup }"/>
								            		</c:forEach>
								            		<!-- 无跳周、跳节排 -->
								            		<c:if test="${currTimetable.timetableAppointmentSameNumbers eq null || fn:length(currTimetable.timetableAppointmentSameNumbers) == 0 }">
									            		...
									            		&& cStatus2.index > currTimetable.startClass/2 && cStatus2.index <= currTimetable.endClass/2}">
										            		...
										            		<c:if test="${empty codeCustom}">
									            				<!-- 合班 -->
											            		<c:if test="${not empty currTimetable.schoolCourseMerge}">
											            			<c:set var="codeCustom" value="${currTimetable.schoolCourseMerge.courseNumber}"/>
											            		</c:if>
											            		<c:if test="${empty currTimetable.schoolCourseMerge}">
						            						...
						            						<c:if test="${sameCount == 0}">
						            							<c:set var="courseNumber" value="${courseNumber}${temp2}<br>"/>
						            						</c:if>
						            						<c:if test="${sameCount != 0}">
						            							<c:set var="courseNumber" value="${courseNumber}<br>${temp2}<br>"/>
						            						</c:if>
								            			...
								            			<c:forEach items = "${currTimetable.timetableAppointmentSameNumbers}" var="currSame">
										            		...
										            		&& cStatus2.index > currSame.startClass/2 && cStatus2.index <= currSame.endClass/2}">
											            		...
											            		<c:if test="${empty codeCustom}">
										            				<!-- 合班 -->
												            		<c:if test="${not empty currTimetable.schoolCourseMerge}">
												            			<c:set var="codeCustom" value="${currSame.timetableAppointment.schoolCourseMerge.courseNumber}"/>
												            		</c:if>
												            		<c:if test="${empty currTimetable.schoolCourseMerge}">
												            			<c:set var="codeCustom" value="${currTimetable.schoolCourseDetail.courseNumber}"/>
												            		</c:if>
		...
		<script type="text/javascript">
			...
			function query(){
				document.form.action="${pageContext.request.contextPath}/newtimetable/myTimetableTeacher";
				document.form.submit();
			}
