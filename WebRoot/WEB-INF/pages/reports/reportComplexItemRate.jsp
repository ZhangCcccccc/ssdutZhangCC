...
<head>
	...
	<c:forEach items="${complexItemRates}" var="curr" varStatus="i">
	    <c:choose>
...
function search()
{
	...
	if(terms != null)
	{
		document.queryForm.action="${pageContext.request.contextPath}/report/reportComplexItemRate";
...
<div class="title">
  <div id="title">报表统计--独立、大型综合性实验课占比 </div>
</div> 
<div class="tool-box">
...
<br />
<div style="text-align:center;"><b>独立、大型综合性实验课占比=大型综合性实验课之和 / 专业数 / 全校大型综合性实验课之和</b></div>
<br />
...
<thead>
  <tr>
    ...
    <th>专业数</th>
    <th>全校大型综合性实验课之和</th>
    <th>独立实验课、大型综合性实验课比例</th>
  </tr>
</thead>
<tbody>
  <c:forEach items="${complexItemRates}" var="curr">
    <tr>
      <td>${curr.labRoomName}</td> <%-- 学院名称 --%>
      <td><a style="color:#0000ff;" href="${pageContext.request.contextPath}/report/reportComplexItems?currpage=1&academyNumber=${curr.labRoomPhone}&terms=${labRoom.labRoomName}">${curr.labRoomAddress}</a></td>
      <td>${curr.labRoomNumber}</td>
      <td><a style="color:#0000ff;" href="${pageContext.request.contextPath}/report/reportComplexItems?currpage=1&academyNumber=&terms=${labRoom.labRoomName}">${curr.labRoomCapacity}</a></td>
