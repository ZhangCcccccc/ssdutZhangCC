...
<link rel="shortcut icon" href="${pageContext.request.contextPath}/images/favicon.gif" />
 <!-- 打印插件的引用 -->
  ...
  <script type="text/javascript">
	//首页
	function first(){
		window.location.href="${pageContext.request.contextPath}/report/reportItems?currpage=1&academyNumber=<%=request.getParameter("academyNumber")%>&terms=<%=request.getParameter("terms")%>";
	}
	//末页
	function last(){
		var totalPage=${pageModel.totalPage};
		window.location.href="${pageContext.request.contextPath}/report/reportItems?currpage="+totalPage+"&academyNumber=<%=request.getParameter("academyNumber")%>&terms=<%=request.getParameter("terms")%>";
	}
	//上一页
	function previous(){
		...
		window.location.href="${pageContext.request.contextPath}/report/reportItems?currpage="+page+"&academyNumber=<%=request.getParameter("academyNumber")%>&terms=<%=request.getParameter("terms")%>";
	}
	//下一页
	function next(){
		var totalPage=${pageModel.totalPage};
...
<div class="page">总共有:${totalRecords}条记录 &nbsp;
	 总页数:${pageModel.totalPage}页 &nbsp; 当前第:${pageModel.currPage}页<input type="hidden" value="${pageModel.lastPage}" id="totalpage" />&nbsp;<!-- 跳转到选择/输入的页面 -->跳转到
		   ...
		   <option value="${pageContext.request.contextPath}/report/reportItems?currpage=<%=request.getParameter("currpage")%>&academyNumber=<%=request.getParameter("academyNumber")%>&terms=<%=request.getParameter("terms")%>">${pageModel.currPage}</option>
		   <c:forEach begin="${pageModel.firstPage}" end="${pageModel.lastPage}" step="1" varStatus="j" var="current">	
           <c:if test="${j.index!=pageModel.currPage}">
           <option value="${pageContext.request.contextPath}/report/reportItems?currpage=${j.index}&academyNumber=<%=request.getParameter("academyNumber")%>&terms=<%=request.getParameter("terms")%>">${j.index}</option>
           </c:if>
           </c:forEach>
           </select> 页
