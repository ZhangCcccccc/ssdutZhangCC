...
<head>
	...
	<c:forEach items="${itemsRates}" var="curr" varStatus="i">
	    <c:choose>
...
<br />
<div style="text-align:center;"><b>实验项目开出率=实际开出实验数 / 计划开设实验数</b></div>
<br />
...
<tbody>
  <c:forEach items="${itemsRates}" var="current" varStatus="i">
  <tr>
  	<td>${current.labRoomName}</td>
  	<td><a style="color:#0000ff;" href="${pageContext.request.contextPath}/report/reportItems?currpage=1&academyNumber=${current.labRoomPhone}&terms=${labRoom.labRoomName}">${current.labRoomAddress}</a></td>
  	<td>${current.labRoomCapacity}</td>
