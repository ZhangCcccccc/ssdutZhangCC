...
<head>
	...
	<c:forEach items="${largeDeviceTimeRates}" var="curr" varStatus="i">
	    <c:choose>
...
<div class="title">
  <div id="title">报表统计--大型设备平均利用率</div>
</div> 
<div class="tool-box">
...
<br />
<div style="text-align:center;"><b>大型设备平均利用率=设备机时 / 大型仪器设备数 / 年平均机时</b></div>
<br />
...
<tbody>
  <c:forEach items="${largeDeviceTimeRates}" var="curr">
    <tr>
      <td>${curr.labRoomAddress}</td> <%-- 学院名称 --%>
      <td>${curr.labRoomNumber}</td>
      <td><a target="_blank" style="color:#0000ff;" href="${pageContext.request.contextPath}/report/reportLargeDeviceAcademy?academyNumber=${curr.labRoomPhone}">${curr.labRoomName}</a></td>
