...
<head>
	...
	<c:forEach items="${teacherItemRates}" var="curr" varStatus="i">
	    <c:choose>
...
function search()
{
	...
	if(terms != null)
	{
		document.queryForm.action="${pageContext.request.contextPath}/report/reportTeacherItemRate";
...
<div class="title">
  <div id="title">报表统计--教师参加实验指导比例 </div>
</div> 
<div class="tool-box">
				<form:form name="queryForm" method="post" modelAttribute="labRoom">
    				...
    				<li><input type="button" value="查询" onclick="search();" />
    				</ul>
				</form:form>
		       </div>
...
<br />
<div style="text-align:center;"><b>教师参加指导比例=参加指导的教师数量 / 学院教师数量</b></div>
<br />
...
<tbody>
  <c:forEach items="${teacherItemRates}" var="current">
  <tr>
  	<td>${current.labRoomAddress}</td>
  	<td>${current.labRoomNumber}</td>
  	<td><a style="color:#0000ff;" href="${pageContext.request.contextPath}/report/reportTeacherItemList?currpage=1&academyNumber=${current.labRoomPhone}&terms=${labRoom.labRoomName}">${current.labRoomName}</a></td>
