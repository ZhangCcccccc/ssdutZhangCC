        ...
        <div class="course_cont r back_gray">
            ...
            <ul class="tool_box tab cf rel zx2 pt5  pb10">
             ...
             <div class="tab_list f14 mb2">
              ...
              <a href="${pageContext.request.contextPath}/tcoursesite/customGrade?currpage=1&tCourseSiteId=${tCourseSite.id }&type=${type }">首页</a>			    
	          <a href="${pageContext.request.contextPath}/tcoursesite/customGrade?currpage=${pageModel.previousPage}&tCourseSiteId=${tCourseSite.id }&type=${type }">上一页</a>
	                                 第<select onchange="javascript:window.location.href = this.options[this.selectedIndex].value;">
	          <option value="${pageContext.request.contextPath}/tcoursesite/customGrade?currpage=${pageModel.currpage}&tCourseSiteId=${tCourseSite.id }&type=${type }">${pageModel.currpage}</option>
	          <c:forEach begin="${pageModel.firstPage}" end="${pageModel.lastPage}" step="1" varStatus="j" var="current">	
              <c:if test="${j.index!=pageModel.currpage}">
              <option value="${pageContext.request.contextPath}/tcoursesite/customGrade?currpage=${j.index}&tCourseSiteId=${tCourseSite.id }&type=${type }">${j.index}</option>
              </c:if>
              </c:forEach></select>页
	          <a href="${pageContext.request.contextPath}/tcoursesite/customGrade?currpage=${pageModel.nextPage}&tCourseSiteId=${tCourseSite.id }&type=${type }">下一页</a>
 	          <a href="${pageContext.request.contextPath}/tcoursesite/customGrade?currpage=${pageModel.lastPage}&tCourseSiteId=${tCourseSite.id }&type=${type }">末页</a>
              </div>
             </div>
...
<script type="text/javascript">
        ...
        function saveCustomScore(id){
           ...
           if ($('#userForm').form('validate')){
       		   form1.action = "${pageContext.request.contextPath }/tcoursesite/userinfo/saveGrading?tCourseSiteId="+id+"&customScoreTitle="+customScoreTitle+"&currpage="+${pageModel.currpage };
               form1.submit();  
           }
        }
