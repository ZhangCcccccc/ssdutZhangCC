    ...
    <div class="course_con ma back_gray">
        <div class="course_cont r back_gray">
            ...
            <div class="tab_list f14 mb2">
              ...
              <a href="${pageContext.request.contextPath}/tcoursesite/examAchievement?currpage=1&tCourseSiteId=${tCourseSite.id }&type=${type }">首页</a>			    
	          <a href="${pageContext.request.contextPath}/tcoursesite/examAchievement?currpage=${pageModel.previousPage}&tCourseSiteId=${tCourseSite.id }&type=${type }">上一页</a>
	                                 第<select onchange="javascript:window.location.href = this.options[this.selectedIndex].value;">
	          <option value="${pageContext.request.contextPath}/tcoursesite/examAchievement?currpage=${pageModel.currpage}&tCourseSiteId=${tCourseSite.id }&type=${type }">${pageModel.currpage}</option>
	          <c:forEach begin="${pageModel.firstPage}" end="${pageModel.lastPage}" step="1" varStatus="j" var="current">	
              <c:if test="${j.index!=pageModel.currpage}">
              <option value="${pageContext.request.contextPath}/tcoursesite/examAchievement?currpage=${j.index}&tCourseSiteId=${tCourseSite.id }&type=${type }">${j.index}</option>
              </c:if>
              </c:forEach></select>页
	          <a href="${pageContext.request.contextPath}/tcoursesite/examAchievement?currpage=${pageModel.nextPage}&tCourseSiteId=${tCourseSite.id }&type=${type }">下一页</a>
 	          <a href="${pageContext.request.contextPath}/tcoursesite/examAchievement?currpage=${pageModel.lastPage}&tCourseSiteId=${tCourseSite.id }&type=${type }">末页</a>
              </div>
            </div>
