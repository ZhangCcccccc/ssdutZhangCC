...
public class JsGridReportBase {
	...
	public void createCell(HSSFRow row, TableColumn tc,
		...
		case TableColumn.COLUMN_TYPE_INTEGER:
			cell.setCellValue(new HSSFRichTextString(String.valueOf(dc.getIntValue())));
		...
		case TableColumn.COLUMN_TYPE_FLOAT_2:
			cell.setCellValue(new HSSFRichTextString(String.valueOf(dc.getDoubleValue())));
			style = styles.get("D2");
			style.setWrapText(true);
			if (row.getRowNum() % 2 != 0)
		...
		case TableColumn.COLUMN_TYPE_RED_BG:
			cell.setCellValue(new HSSFRichTextString(dc.getValue()));
